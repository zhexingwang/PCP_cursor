...
import paths_v3 as paths
from competitive_adsorption_v3 import simulate
from params_columns_v3 import (
...
from parameter_fitting_v3 import apply_fitted_vector, fit_parameters_batches, load_fitted_vector, save_fit_result
from evaluate_model_accuracy_v3 import evaluate_model_batches
...
def main():
    ap = argparse.ArgumentParser(description="Fit R004A using R003 effluent as inlet (v3)")
    ...
    if not r003_fit_path.exists():
        raise SystemExit(f"FileNotFoundError: {r003_fit_path} — run run_fit_R003_v3.py first")
    ...
    for bid in sorted(set(train_ids + test_ids)):
        ...
        pr.C_in = {**pr.C_in, **Cin_from_process_row(dfp, pr.species_names)}
        flow = flow_df_from_process(dfp)
        _, effluent, _, _ = simulate(pr, v_T_series=flow)
        from competitive_adsorption_v3 import effluent_to_Cin_series
    ...
    extra = {
        ...
        "based_on_R003": {k: fitted_r003.get(k) for k in ("q_total", "eps_b", "keys", "values")},
